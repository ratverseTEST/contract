...
contract TESTnft is Ownable, ERC721, ERC721Enumerable {
    ...
    uint private _decimalBase = 3;
    uint private _marketingMint;
    uint private _maxMintPerTx;
    uint private _maxSupply;
    uint private _mintPrice;
    uint private _reflection;
    ...
    // mint shares
    uint private _mintSharesTeam = 750;
    uint private _mintSharesReflect = 200;
    uint private _mintSharesLottery = 50;

    // balances
    uint private _balanceLotteryPot;
    uint private _balanceTeam;
    ...
    event TokensMinted(uint currentSupply, uint maxSupply, uint reflectBalance, uint lotteryPot);
    ...
    ERC721(name, symbol)
    {
        // set basic info for contract
        _maxSupply = maxSupply;
        _mintPrice = mintPrice;
        _maxMintPerTx = maxMintPerTx;
        _assetsBaseURI = assetsBaseURI;
        _reflection = (_actualMintPrice() * _mintSharesReflect) / (10 ** _decimalBase);

        // mint for marketing purposes
        _marketingMint = marketingMint;
        mintToken(owner(), marketingMint, 0);
    }
    ...
    // -- utils -- //
    function _actualMintPrice() internal view returns (uint) {
        return _mintPrice * (10 ** (18 - _decimalBase));
    }
     ...
     * _mintSharesLottery: to allocate toward lottery pot
     */
    function _splitMintFees(uint payment) internal {
        if (payment > 0) {
            uint reflect = (payment * _mintSharesReflect) / (10 ** _decimalBase);
            uint lottery = (payment * _mintSharesLottery) / (10 ** _decimalBase);

            _balanceReflections += reflect;
            _balanceLotteryPot += lottery;
            _balanceTeam += payment - (reflect + lottery);
        }
    }
     ...
     */
    function teamPayout() public onlyOwner {
        payable(owner()).transfer(_balanceTeam);
    }

    /**
     * @dev set a lottery wallet. funds for lottery pot will go here and
     * get distributed to winners. This isolate all transactions in one place
     */
    function setLotteryWallet(address lWallet) public onlyOwner {
        _lotteryWallet = lWallet;
    }

    /**
     * @dev send all funds allocated for lottery pot to lottery wallet
     */
    function lotteryPayout() public onlyOwner {
        uint p = _balanceLotteryPot;
        require((_lotteryWallet != address(0)) && (p > 0) && (p <= address(this).balance), "No wallet set or insufficient balance");
        payable(_lotteryWallet).transfer(p);
        _balanceLotteryPot = 0;
    }
     ...
     * to all holders.
     */
    function airdropMint(address recipient, uint amount) public onlyOwner {
     ...
     */
    function setAssetsBaseURI(string memory baseURI) public onlyOwner {
     ...
     * a fair-launch
     */
    function setMaxMintPerTx(uint amount) public onlyOwner {
        _maxMintPerTx = amount;
    }
     ...
     */
    function setMaxSupply(uint max) public onlyOwner {
    ...
    function setMintPrice(uint price) public onlyOwner {
        require(price <= (10 ** _decimalBase), "Must be between 1-1000");
        _mintPrice = price;
        _reflection = (_actualMintPrice() * _mintSharesReflect) / (10 ** _decimalBase);
    }
    ...
    function mintToken(address recipient, uint amount, uint payment) internal {
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (payment > 0) {
                _totalDividend += (_reflection / tokenId);
            }
        }
        emit TokensMinted(totalSupply(), _maxSupply, _balanceReflections, _balanceLotteryPot);
    }
     ...
     */
    function cData() public view returns (bool, uint, uint, uint, uint, uint, uint, uint, string memory, address) {
        return (_mintingStarted, _decimalBase, totalSupply(), _maxSupply, _mintPrice, _reflection, _maxMintPerTx, _marketingMint, _assetsBaseURI, _lotteryWallet);
    }

    /**
     * @dev mint shares and distribution
     */
    function cShares() public view returns (uint, uint, uint) {
        return (_mintSharesReflect, _mintSharesLottery, _mintSharesTeam);
    }
     ...
     * @dev account data
     */
    function claimelbleDivs() public view returns (uint256) {
        return _allEarnedReflection(_msgSender());
    }

    function claimelbleDivsMan(address userAddress) public view returns (uint256) {
        return _allEarnedReflection(userAddress);
    }
    /**
